...
from model.networks import Discriminator, Generator
from utils.data import ImageDataset

parser = argparse.ArgumentParser()
parser.add_argument(
    "--config", type=str, default="configs/train.yaml", help="Path to yaml config file"
)
...
def training_loop(
    ...
    for n_iter in range(init_n_iter, config.max_iters):
        ...
        batch_filled_mask = torch.cat(
            (batch_complete.detach(), torch.tile(mask, [config.batch_size, 1, 1, 1])),
            dim=1,
        )
        ...
        losses["ae_loss2"] = config.l1_loss_alpha * torch.mean(
            (torch.abs(batch_real - x2))
        )
        losses["ae_loss"] = losses["ae_loss1"] + losses["ae_loss2"]

        batch_gen = batch_complete
        batch_gen = torch.cat(
            (batch_gen, torch.tile(mask, [config.batch_size, 1, 1, 1])), dim=1
        )
        ...
        if (
            ...
            img_grids = [
                tv.utils.make_grid(images[: config.viz_max_out], nrow=2)
        ...
        if (
            ...
            tv.utils.save_image(
                img_grids, f"{config.checkpoint_dir}/images/iter_{n_iter}.png", nrow=2
            )
...
def main():
    ...
    # transforms
    transforms = (
        [T.RandomHorizontalFlip(0.5)] if config.random_horizontal_flip else None
    )
    ...
    generator = Generator(
        cnum_in=cnum_in + 2, cnum_out=cnum_in, cnum=48, return_flow=False
    )
